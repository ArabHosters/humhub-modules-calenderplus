 ...
 */
class CalendarPlusEntry extends HActiveRecordContent
{
     ...
     * @return CalendarPlusEntry the static model class
     */
    ...
    {
        return array(
            'id' => Yii::t('CalendarPlusModule.base', 'ID'),
            'title' => Yii::t('CalendarPlusModule.base', 'Title'),
            'description' => Yii::t('CalendarPlusModule.base', 'Description'),
            'start_time' => Yii::t('CalendarPlusModule.base', 'Start Date and Time'),
            'end_time' => Yii::t('CalendarPlusModule.base', 'End Date and Time'),
            'start_time_date' => Yii::t('CalendarPlusModule.base', 'Start Date'),
            'end_time_date' => Yii::t('CalendarPlusModule.base', 'End Date'),
            'all_day' => Yii::t('CalendarPlusModule.base', 'All Day'),
            'participation_mode' => Yii::t('CalendarPlusModule.base', 'Participation Mode'),
            'recur' => Yii::t('CalendarPlusModule.base', 'Recur'),
            'recur_type' => Yii::t('CalendarPlusModule.base', 'Recur Type'),
            'recur_interval' => Yii::t('CalendarPlusModule.base', 'Recur Interval'),
            'recur_end' => Yii::t('CalendarPlusModule.base', 'Recur End'),
        );
    }
    ...
    public function validateEndTime($attribute, $params)
    {
        ...
        if ($this->start_time != "" && $this->end_time != "") {
            ...
            if ($s > $e) {
                $this->addError($attribute, Yii::t('CalendarPlusModule.base', "End time must be after start time!"));
            }
        }
    }
    ...
    public static function getContainerEntriesByRange(DateTime $start, DateTime $end, HActiveRecordContentContainer $contentContainer, $limit = 0)
    {
        ...
        foreach (CalendarPlusEntry::model()->contentContainer($contentContainer)->findAll($criteria) as $entry) {
    ...
    public static function getEntriesByRange(DateTime $start, DateTime $end, $includes = array(), $filters = array(), $limit = 0)
    {
        ...
        $criteria->condition .= ' AND c.object_model="CalendarPlusEntry"';
        ...
        if (in_array(self::FILTER_PARTICIPATE, $filters)) {
            $criteria->condition .= " AND p.participation_state=" . CalendarPlusEntryParticipant::PARTICIPATION_STATE_ACCEPTED;
        }
        if (in_array(self::FILTER_INVITED, $filters)) {
            $criteria->condition .= " AND p.participation_state=" . CalendarPlusEntryParticipant::PARTICIPATION_STATE_INVITED;
        }
    ...
    public static function getUpcomingEntries(HActiveRecordContentContainer $contentContainer = null, $daysInFuture = 7, $limit = 5)
    {
        ...
        if ($contentContainer == null) {
            ...
            $criteria->condition .= ' AND p.user_id=' . Yii::app()->user->id . ' AND p.participation_state=' . CalendarPlusEntryParticipant::PARTICIPATION_STATE_ACCEPTED;
        }
    ...
    public function beforeDelete()
    {
        ...
        foreach (CalendarPlusEntryParticipant::model()->findAllByAttributes(array('calendar_entry_id' => $this->id)) as $participant) {
            $participant->delete();
        }
    ...
    public function beforeSave()
    {
        ...
        if (CalendarPlusUtils::isFullDaySpan($startTime, $endTime)) {
            $this->all_day = 1;
        } else {
            $this->all_day = 0;
        }
    ...
    public function afterSave()
    {
        ...
        if ($this->isNewRecord) {
            ...
            $participant = new CalendarPlusEntryParticipant;
            $participant->user_id = Yii::app()->user->id;
            $participant->calendar_entry_id = $this->id;
            $participant->participation_state = CalendarPlusEntryParticipant::PARTICIPATION_STATE_ACCEPTED;
            $participant->save();
    ...
    public function getFullCalendarArray()
    {
        ...
        return array(
            ...
            'updateUrl' => $this->createContainerUrlTemp('//calendarplus/entry/editAjax', array('id' => $this->id, 'end_time' => '-end-', 'start_time' => '-start-', 'fullCalendar' => '1')),
            'viewUrl' => $this->createContainerUrlTemp('//calendarplus/entry/view', array('id' => $this->id, 'fullCalendar' => '1')),
    ...
    public function canRespond(User $user = null)
    {
        ...
        if ($this->participation_mode == self::PARTICIPATION_MODE_INVITE) {
            $participant = CalendarPlusEntryParticipant::model()->findByAttributes(array('calendar_entry_id' => $this->id, 'user_id' => $user->id));
            if ($participant !== null) {
                return true;
            }
        }
    ...
    {
        return Yii::t('CalendarPlusModule.base', "Event") . " \"" . Helpers::truncateText($this->title, 25) . "\"";
    }
